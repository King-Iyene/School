  ...
  return (
    <div className="p-6">
      ...
      {loading ? (
            ...
            <table className="w-full text-sm border-collapse min-w-[800px]">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200">
                  <th className="px-4 py-4 text-left font-semibold text-slate-600 border-r border-slate-200">
                    Period / Time
                  </th>
                  {weekDays.map((day) => (
                    <th key={day.id} className={`px-4 py-4 text-center font-semibold border-r border-slate-200 last:border-r-0 ${day.is_weekend ? 'bg-slate-100 text-slate-500' : 'text-slate-600'}`}>
                      {day.name}
              ...
              <tbody className="divide-y divide-slate-100">
                {timeSlots.length === 0 ? (
                  <tr>
                    <td colSpan={weekDays.length + 1} className="text-center py-12 text-slate-400">
                  ...
                  timeSlots.map((slot) => (
                    <tr key={slot.id} className="hover:bg-slate-50/50 transition-colors group">
                      <td className="px-4 py-3 font-medium text-slate-800 border-r border-slate-100 bg-slate-50/30 whitespace-nowrap">
                        <div className="text-base text-slate-700">{slot.period_name}</div>
                        <div className="text-xs text-slate-400">{formatTime(slot.start_time)} - {formatTime(slot.end_time)}</div>
                      </td>
                      {weekDays.map((day) => {
                        const entry = getRoutineForCell(slot.id, day.id);
                        if (day.is_weekend) {
                          return (
                            <td key={day.id} className="px-2 py-3 text-center border-r border-slate-100 last:border-r-0 align-middle bg-slate-50/50">
                        ...
                        return (
                          <td key={day.id} className="px-2 py-3 text-center border-r border-slate-100 last:border-r-0 align-top relative min-w-[140px]">
                            {entry ? (
                                ...
                                {entry.is_break ? (
                                 ...
                                 {isSuperAdmin && (
                                   <div className="absolute bottom-1 right-1 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                      <button onClick={() => openCellModal(slot.id, day.id)} className="p-1 bg-white border border-slate-200 text-slate-400 hover:text-emerald-500 rounded-md shadow-sm">
                                        <Edit2 size={12} />
                                      </button>
                                      <button onClick={() => handleDeleteCell(entry.id)} className="p-1 bg-white border border-slate-200 text-slate-400 hover:text-red-500 rounded-md shadow-sm">
                                        <Trash2 size={12} />
                                      </button>
                                   </div>
                                 )}
        ...
        <div className="bg-white rounded-2xl border border-slate-200 flex flex-col items-center justify-center h-64 text-slate-400 gap-3 shadow-sm">
      ...
      >
        <div className="space-y-5">
           <div className="flex items-center gap-3 pb-2 border-b border-slate-100">
             ...
             <span className="text-sm font-semibold text-slate-800">Is this a Break?</span>
           </div>
           ...
           <div className={`space-y-4 transition-all duration-300 ${isBreak ? 'blur-[2px] opacity-40 pointer-events-none' : ''}`}>
             <div>
               ...
               <p className="text-xs text-slate-400 mt-1">Subjects filtered by class level.</p>
             </div>
           ...
           <div className="bg-slate-50 border border-slate-100 rounded-xl p-3 flex items-start gap-3 mt-2">
             <input 
                type="checkbox" 
                id="applyAll" 
                checked={applyAll}
             ...
             />
             <label htmlFor="applyAll" className="text-sm text-slate-700 cursor-pointer">
               <span className="font-medium block mb-0.5">Apply for all weekdays</span>
               <span className="text-xs text-slate-500">This will copy the same data across {weekDays.map(d => d.name).join(', ')} for this period.</span>
             </label>
           </div>

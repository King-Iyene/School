    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 text-center">
          ...
          <p className="text-2xl font-bold text-slate-800">{examsTaken}</p>
          <p className="text-xs text-slate-500 mt-0.5">Exams Taken</p>
        </div>
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 text-center">
          <Award className="w-6 h-6 text-blue-500 mx-auto mb-2" />
          <p className="text-2xl font-bold text-slate-800">{overallAvg}</p>
          <p className="text-xs text-slate-500 mt-0.5">Overall Average</p>
        </div>
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 text-center">
          <BookOpen className="w-6 h-6 text-emerald-500 mx-auto mb-2" />
          <p className="text-sm font-bold text-slate-800 truncate">{bestSubject?.name || '—'}</p>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 text-center">
          ...
          <p className="text-xs text-slate-500 mt-0.5">Upcoming Exams</p>
        </div>
      </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50">
                  ...
                  <th className="text-left px-5 py-3 text-xs font-semibold text-slate-500 uppercase">Exam</th>
                  ...
                  <th className="text-left px-5 py-3 text-xs font-semibold text-slate-500 uppercase">Room</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {upcoming.map(item => (
                  <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-5 py-3.5 font-medium text-slate-800">{(item.subjects as any)?.name || '—'}</td>
                    <td className="px-5 py-3.5 text-slate-600">{(item.exams as any)?.name || '—'}</td>
                    ...
                    <td className="px-5 py-3.5 text-slate-600">{item.duration ? `${item.duration} min` : '—'}</td>
                    <td className="px-5 py-3.5 text-slate-600">
                      <div className="flex items-center gap-1.5">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-5 border-b border-slate-100 flex items-center gap-2">
          ...
          <h2 className="font-semibold text-slate-800">My Results</h2>
          <span className="ml-auto text-xs bg-slate-100 text-slate-600 font-semibold px-2.5 py-1 rounded-full">{examsTaken} exam{examsTaken !== 1 ? 's' : ''}</span>
        </div>
        ...
        {examGroups.length === 0 ? (
          ...
          <>
            <div className="px-5 pt-4 flex gap-2 flex-wrap border-b border-slate-100 pb-4">
              {examGroups.map(g => (
                <button
                  key={g.exam_id}
              ...
              <>
                <div className="grid grid-cols-3 divide-x divide-slate-100 border-b border-slate-100">
                  <div className="p-4 text-center">
                    <p className="text-lg font-black text-slate-800">{selectedGroup.aggregate}</p>
                  ...
                  <div className="p-4 text-center">
                    ...
                    <p className="text-xs text-slate-400">Average (out of 100)</p>
                  </div>
                  <div className="p-4 text-center">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-slate-50">
                        <th className="text-left px-5 py-3 text-xs font-semibold text-slate-500 uppercase">Subject</th>
                        <th className="text-center px-4 py-3 text-xs font-semibold text-slate-500 uppercase">CA (10)</th>
                        <th className="text-center px-4 py-3 text-xs font-semibold text-slate-500 uppercase">Test (30)</th>
                        <th className="text-center px-4 py-3 text-xs font-semibold text-slate-500 uppercase">Exam (60)</th>
                        <th className="text-center px-4 py-3 text-xs font-semibold text-slate-500 uppercase">Total (100)</th>
                        <th className="text-center px-4 py-3 text-xs font-semibold text-slate-500 uppercase">Grade</th>
                        <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 uppercase">Remark</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                      {selectedGroup.subjects.map((s, idx) => (
                        <tr key={idx} className={`hover:bg-slate-50 transition-colors ${s.is_absent ? 'bg-red-50/30' : ''}`}>
                          <td className="px-5 py-3.5 font-medium text-slate-800">{s.subject_name}</td>
                          <td className="px-4 py-3.5 text-center text-slate-600">{s.is_absent ? '—' : s.ca}</td>
                          <td className="px-4 py-3.5 text-center text-slate-600">{s.is_absent ? '—' : s.test}</td>
                          <td className="px-4 py-3.5 text-center text-slate-600">{s.is_absent ? '—' : s.exam}</td>
                          <td className="px-4 py-3.5 text-center">
                            <span className={`font-black ${s.is_absent ? 'text-slate-300' : 'text-slate-800'}`}>
                          ...
                          <td className="px-4 py-3.5 text-slate-500 text-sm">{s.remark}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>

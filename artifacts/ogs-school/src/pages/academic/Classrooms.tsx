    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Manage classrooms and their details</p>
        </div>
        <button
          onClick={openAdd}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                <th className="text-left px-4 py-3 font-medium text-slate-600">Class Name</th>
            ...
            <tbody>
              {classrooms.length === 0 ? (
                <tr>
                  <td colSpan={5} className="text-center py-10 text-slate-400">No classrooms found</td>
                </tr>
              ) : (
                classrooms.map((classroom) => (
                  <tr key={classroom.id} className="border-b border-slate-100 hover:bg-slate-50">
                    <td className="px-4 py-3 font-medium text-slate-800">{classroom.room_no}</td>
                    <td className="px-4 py-3 text-slate-600">{classroom.building || '-'}</td>
                    <td className="px-4 py-3 text-slate-600">{classroom.capacity ?? '-'}</td>

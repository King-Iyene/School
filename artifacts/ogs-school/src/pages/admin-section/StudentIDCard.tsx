    ...
    return (
      <div className="space-y-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-800">Generate ID Cards</h2>
            <p className="text-slate-500 text-sm">Select a template and class to generate ID cards</p>
          </div>
        </div>
        ...
        {selectedTemplate && genClass && (
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-slate-100">
              <p className="font-semibold text-slate-800 text-sm">Students — {selectedTemplate.title}</p>
              {students.length > 0 && (
                <div className="flex items-center gap-3">
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-slate-800">Student ID Cards</h2>
          <p className="text-slate-500 text-sm">Manage ID card templates and generate cards for students</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          {templates.map(t => (
            <div key={t.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden group">
              ...
              <div className="p-4">
                ...
                {t.designation && <p className="text-xs text-slate-500 mt-0.5">{t.designation}</p>}
                <div className="flex items-center gap-2 mt-2">
                  <div className="w-3 h-3 rounded-full border border-slate-200" style={{ backgroundColor: t.background_color }} />
                  <div className="w-3 h-3 rounded-full" style={{ backgroundColor: t.accent_color }} />
                  {t.signature_url && <span className="text-xs text-slate-400">With signature</span>}
                </div>
                <button
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <div className="flex items-center gap-2">
                <input type="color" value={form.background_color} onChange={e => setForm({ ...form, background_color: e.target.value })} className="w-10 h-10 rounded-lg border border-slate-200 cursor-pointer p-0.5" />
                <input value={form.background_color} onChange={e => setForm({ ...form, background_color: e.target.value })} className="flex-1 border border-slate-200 rounded-xl px-3 py-2 text-xs focus:outline-none" />
              </div>
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Accent Color</label>
              <div className="flex items-center gap-2">
                <input type="color" value={form.accent_color} onChange={e => setForm({ ...form, accent_color: e.target.value })} className="w-10 h-10 rounded-lg border border-slate-200 cursor-pointer p-0.5" />
                <input value={form.accent_color} onChange={e => setForm({ ...form, accent_color: e.target.value })} className="flex-1 border border-slate-200 rounded-xl px-3 py-2 text-xs focus:outline-none" />
              </div>
            </div>
          </div>

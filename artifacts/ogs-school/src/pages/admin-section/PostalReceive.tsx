  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-slate-800">Postal Receive</h2>
          <p className="text-slate-500 text-sm">Log all incoming postal correspondence</p>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm">
        <div className="p-4 border-b border-slate-100">
          <div className="relative">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by sender, reference, subject..." className="w-full pl-9 pr-4 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/30" />
          </div>
        </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                  <td className="px-5 py-3 text-sm font-medium text-slate-800">{item.from_title}</td>
                  <td className="px-5 py-3 text-sm text-slate-600 font-mono">{item.reference_no || '—'}</td>
                  <td className="px-5 py-3 text-sm text-slate-500 truncate max-w-[160px]">{item.from_address || '—'}</td>
                  <td className="px-5 py-3 text-sm text-slate-600">{item.received_by || '—'}</td>
                  <td className="px-5 py-3 text-sm text-slate-500">{item.notes || '—'}</td>

  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-app-surface rounded-2xl border border-app-border shadow-sm overflow-hidden">
        <table className="w-full">
          <thead>
            <tr className="border-b border-app-border bg-app-surface-alt">
              <th className="text-left text-xs font-semibold text-app-text-muted uppercase px-5 py-3">Type</th>
              <th className="text-left text-xs font-semibold text-app-text-muted uppercase px-5 py-3">Caller</th>
              <th className="text-left text-xs font-semibold text-app-text-muted uppercase px-5 py-3">Subject</th>
              <th className="text-left text-xs font-semibold text-app-text-muted uppercase px-5 py-3">Duration</th>
              <th className="text-left text-xs font-semibold text-app-text-muted uppercase px-5 py-3">Handled By</th>
          ...
          <tbody className="divide-y divide-app-border">
            {loading ? (
              ...
              <tr><td colSpan={8} className="text-center py-8 text-app-text-muted">No call logs found</td></tr>
            ) : filtered.map(c => (
              <tr key={c.id} className="hover:bg-app-surface-alt transition-colors">
                ...
                <td className="px-5 py-3">
                  <p className="text-sm font-medium text-app-text">{c.caller_name}</p>
                  <p className="text-xs text-app-text-muted">{c.phone}</p>
                </td>
                <td className="px-5 py-3 text-sm text-app-text-muted">{c.subject || '—'}</td>
                <td className="px-5 py-3 text-sm text-app-text-muted">{c.duration_minutes ? `${c.duration_minutes} min` : '—'}</td>
                <td className="px-5 py-3 text-sm text-app-text-muted">{c.handled_by || '—'}</td>
                <td className="px-5 py-3 text-sm text-app-text-muted">{c.next_follow_up_date ? new Date(c.next_follow_up_date).toLocaleDateString() : '—'}</td>
                <td className="px-5 py-3 text-sm text-app-text-muted">{new Date(c.call_date).toLocaleDateString()}</td>
                <td className="px-5 py-3">
        ...
        <div className="px-5 py-3 border-t border-app-border text-sm text-app-text-muted">{filtered.length} of {calls.length} call logs</div>
      </div>
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={editItem ? 'Edit Call Log' : 'Log Phone Call'} size="lg">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="block text-sm font-medium text-app-text mb-1">Call Type</label>
              <select value={form.call_type} onChange={e => setForm({ ...form, call_type: e.target.value })} className={`${inputCls} bg-app-surface`}>
                <option value="incoming">Incoming</option>
                <option value="outgoing">Outgoing</option>
              </select>
            </div>
            <div>
              <label className="block text-sm font-medium text-app-text mb-1">Caller / Contact Name *</label>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="block text-sm font-medium text-app-text mb-1">Call Date</label>
              <input type="date" value={form.call_date} onChange={e => setForm({ ...form, call_date: e.target.value })} className={inputCls} />
            </div>
            <div>
              <label className="block text-sm font-medium text-app-text mb-1">Duration (minutes)</label>
            ...
            <div>
              <label className="block text-sm font-medium text-app-text mb-1">Next Follow-up</label>
          ...
          <div>
            <label className="block text-sm font-medium text-app-text mb-1">Handled By</label>

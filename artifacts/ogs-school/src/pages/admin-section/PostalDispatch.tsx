  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-app-text">Postal Dispatch</h2>
          <p className="text-app-text-muted text-sm">Log all outgoing postal correspondence</p>
        </div>
      ...
      <div className="bg-app-surface rounded-2xl border border-app-border shadow-sm">
        <div className="p-4 border-b border-app-border">
          <div className="relative">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by recipient, reference, subject..." className="bg-app-surface text-app-text w-full pl-9 pr-4 py-2 border border-app-border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-app-primary/30" />
          </div>
        </div>
            ...
            <tbody className="divide-y divide-app-border">
              {loading ? (
                ...
                <tr key={item.id} className="hover:bg-app-surface-alt transition-colors">
                  <td className="px-5 py-3 text-sm font-medium text-app-text">{item.to_title}</td>
                  <td className="px-5 py-3 text-sm text-app-text-muted font-mono">{item.reference_no || '—'}</td>
                  <td className="px-5 py-3 text-sm text-app-text-muted truncate max-w-[160px]">{item.address || '—'}</td>
                  <td className="px-5 py-3 text-sm text-app-text-muted">{item.from_title || '—'}</td>
                  <td className="px-5 py-3 text-sm text-app-text-muted">{item.note || '—'}</td>
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={editItem ? 'Edit Record' : 'Add Postal Dispatch'} size="lg">
        <div className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button onClick={handleSave} disabled={saving || !form.to_title} className="flex-1 px-4 py-2 bg-app-primary hover:opacity-90 text-white rounded-xl text-sm font-medium disabled:opacity-50">{saving ? 'Saving...' : editItem ? 'Update' : 'Add Record'}</button>
          </div>
        </div>
      </Modal>
    </div>

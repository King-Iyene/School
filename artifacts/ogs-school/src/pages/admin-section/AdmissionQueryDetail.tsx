...
export default function AdmissionQueryDetail({ query, onClose, onEdit, onDelete, onAddFollowUp }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-app-border bg-app-surface-alt">
          <div>
            <h2 className="text-lg font-bold text-app-text">{query.student_name}</h2>
            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium mt-1 ${statusColors[query.status] ?? 'bg-slate-100 text-app-text-muted'}`}>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-app-text text-xs font-medium rounded-lg transition-colors"
            >
              <Edit2 className="w-3.5 h-3.5" /> Edit
            </button>
            <button onClick={onClose} className="p-1.5 text-app-text-muted hover:text-app-text hover:bg-slate-100 rounded-lg transition-colors">
              <X className="w-5 h-5" />
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="p-6 space-y-6">
            ...
            {(query.description || query.note) && (
              <div className="bg-app-surface-alt rounded-xl p-4 border border-app-border">
                <div className="flex items-center gap-2 mb-2">
                  <MessageSquare className="w-4 h-4 text-app-text-muted" />
                  <span className="text-xs font-semibold text-app-text-muted uppercase tracking-wide">Notes / Description</span>
                </div>
                <p className="text-sm text-app-text leading-relaxed whitespace-pre-wrap">{query.description || query.note}</p>
              </div>
            )}

            <div>
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-semibold text-app-text">Follow-up History</h3>
                <span className="text-xs text-app-text-muted">{followUps.length} record{followUps.length !== 1 ? 's' : ''}</span>
              </div>
                      ...
                      <div key={f.id} className="relative pl-10">
                        ...
                        <div className="bg-app-surface border border-app-border rounded-xl px-4 py-3">
                          <div className="flex items-start justify-between gap-2">
                            <p className="text-sm text-app-text leading-relaxed">{f.note}</p>
                            <button
        ...
        <div className="px-6 py-4 border-t border-app-border bg-app-surface-alt">
          <button
...
function InfoItem({ icon, label, value, highlight }: { icon: React.ReactNode; label: string; value: string; highlight?: boolean }) {
  return (
    <div>
      ...
      <p className={`text-sm font-medium pl-6 ${highlight ? 'text-blue-600' : 'text-app-text'}`}>{value}</p>
    </div>
  );
}

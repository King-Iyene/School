...
const INPUT = 'border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/30 w-full bg-white';
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm">Master tracker of all school property — furniture, equipment, bedding and more</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          { label: 'Total Items', value: stats.total, icon: Package, bg: 'bg-slate-50', text: 'text-slate-700', iconBg: 'bg-slate-200' },
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 space-y-3">
        <div className="flex gap-2">
          ...
          <button onClick={() => setShowFilters(f => !f)}
            className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border text-sm font-medium transition-all ${showFilters || activeFilters > 0 ? 'border-emerald-400 bg-emerald-50 text-emerald-700' : 'border-slate-200 text-slate-600 hover:border-slate-300'}`}>
          ...
          {activeFilters > 0 && (
            <button onClick={() => { setFilterLoc(''); setFilterRoom(''); setFilterCat(''); setFilterCond(''); }}
              className="flex items-center gap-1.5 px-3 py-2.5 rounded-xl border border-slate-200 text-sm text-slate-500 hover:text-red-500 hover:border-red-200 transition-colors">
        ...
        {showFilters && (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 pt-1">
            ...
            <div>
              <label className="block text-xs font-medium text-slate-500 mb-1">Room / Office</label>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-5 py-3 border-b border-slate-100 flex items-center justify-between">
          <span className="text-xs font-semibold text-slate-500">{filteredAssets.length} item{filteredAssets.length !== 1 ? 's' : ''} {search || activeFilters > 0 ? 'found' : 'total'}</span>
        </div>

        {loading ? (
          <div className="py-16 text-center text-slate-400 text-sm">Loading assets…</div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b border-slate-100">
                <tr>
                  <th className="text-left px-5 py-3 font-semibold text-slate-600 text-xs uppercase tracking-wide">Item</th>
                  ...
                  <th className="text-left px-4 py-3 font-semibold text-slate-600 text-xs uppercase tracking-wide">Room / Office</th>
                  <th className="text-center px-4 py-3 font-semibold text-slate-600 text-xs uppercase tracking-wide">Qty</th>
                  <th className="text-center px-4 py-3 font-semibold text-slate-600 text-xs uppercase tracking-wide">Condition</th>
                  <th className="text-right px-5 py-3 font-semibold text-slate-600 text-xs uppercase tracking-wide">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {filteredAssets.map((a, idx) => (
                  <tr key={a.id} className={`hover:bg-slate-50/70 transition-colors ${idx % 2 === 1 ? 'bg-slate-50/30' : ''}`}>
                    <td className="px-5 py-3">
                      <div className="font-medium text-slate-800">{a.name}</div>
                      {a.serial_number && <div className="text-xs text-slate-400 mt-0.5">S/N: {a.serial_number}</div>}
                    ...
                    <td className="px-4 py-3">
                      {(a.asset_categories as any)?.name
                        ? <span className="text-xs font-medium px-2.5 py-1 rounded-full bg-slate-100 text-slate-600">{(a.asset_categories as any).name}</span>
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className="text-base font-bold text-slate-800">{a.quantity}</span>
                    </td>
                    <td className="px-4 py-3 text-center">
                    ...
                    <td className="px-5 py-3">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => openEdit(a)}
                          className="p-1.5 rounded-lg text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 transition-colors" title="Edit">
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editId ? 'Edit Asset' : 'Add Asset'}>
        <form onSubmit={handleSubmit} className="space-y-4 p-1">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1">Building / Block</label>
            ...
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1">Room / Office</label>
              ...
              <div className="grid grid-cols-4 gap-2">
                {CONDITIONS.map(c => (
                  <button type="button" key={c.value} onClick={() => setF('condition', c.value)}
                    className={`py-2 rounded-xl border-2 text-xs font-semibold transition-all ${form.condition === c.value ? `${c.bg} ${c.text} border-current` : 'border-slate-200 text-slate-500 hover:border-slate-300'}`}>
                    {c.label}
                  </button>
                ))}
              </div>
            ...
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1">Serial / Tag No.</label>
              <input value={form.serial_number} onChange={e => setF('serial_number', e.target.value)} placeholder="Optional" className={INPUT} />
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1">Unit Value (₦)</label>
          ...
          <div className="flex justify-end gap-2 pt-1">
            <button type="button" onClick={() => setModalOpen(false)} className="px-4 py-2 rounded-xl border border-slate-200 text-sm text-slate-600 hover:bg-slate-50">Cancel</button>
      ...
      <Modal isOpen={!!deleteId} onClose={() => setDeleteId(null)} title="Delete Asset">
        <div className="p-1 space-y-4">
          <p className="text-sm text-slate-600">Are you sure you want to remove this asset from the register? This cannot be undone.</p>
          <div className="flex justify-end gap-2">
            <button onClick={() => setDeleteId(null)} className="px-4 py-2 rounded-xl border border-slate-200 text-sm text-slate-600 hover:bg-slate-50">Cancel</button>

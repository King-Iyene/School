...
export default function AssetLocations() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-xl font-bold text-slate-800">Locations & Rooms</h2>
        <p className="text-slate-500 text-sm">Define buildings, blocks and the rooms/offices inside them</p>
      </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-semibold text-slate-800">Buildings / Blocks</h3>
              <span className="text-xs px-2 py-0.5 rounded-full bg-slate-100 text-slate-500 font-medium">{locations.length}</span>
            </div>
          ...
          {loading ? (
            ...
            <div className="py-14 text-center">
              ...
              <p className="text-slate-400 text-sm">No buildings added yet</p>
            </div>
          ) : (
              ...
              {locations.map(l => {
                ...
                return (
                  <div key={l.id} onClick={() => setSelectedLoc(l.id)}
                    className={`flex items-center gap-3 px-5 py-3.5 cursor-pointer transition-colors group ${isSelected ? 'bg-emerald-50 border-l-2 border-emerald-500' : 'hover:bg-slate-50 border-l-2 border-transparent'}`}>
                    <div className="flex-1 min-w-0">
                      <div className={`font-medium text-sm ${isSelected ? 'text-emerald-700' : 'text-slate-700'}`}>{l.name}</div>
                      <div className="text-xs text-slate-400 mt-0.5">{l.type} · {roomCount} room{roomCount !== 1 ? 's' : ''}</div>
                    </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              {selectedLocation && (
                <span className="text-xs px-2 py-0.5 rounded-full bg-slate-100 text-slate-500 font-medium">{locRooms.length}</span>
              )}
            </div>
          ...
          {!selectedLoc ? (
            <div className="py-14 text-center">
              ...
              <p className="text-slate-400 text-sm">Select a building to manage its rooms</p>
            </div>
          ) : locRooms.length === 0 ? (
            <div className="py-14 text-center">
              <DoorOpen className="w-10 h-10 text-slate-200 mx-auto mb-2" />
              <p className="text-slate-400 text-sm">No rooms in this building yet</p>
              <button onClick={openAddRoom} className="mt-3 text-xs text-blue-600 underline">Add first room</button>
            </div>
          ) : (
            <div className="divide-y divide-slate-100">
              {locRooms.map(r => (
                <div key={r.id} className="flex items-center gap-3 px-5 py-3 group hover:bg-slate-50 transition-colors">
                  <div className="flex-1 min-w-0">
      ...
      <Modal isOpen={locModal} onClose={() => setLocModal(false)} title={editLocId ? 'Edit Building' : 'Add Building'}>
        <form onSubmit={handleLocSubmit} className="p-1 space-y-4">
          ...
          <div className="flex justify-end gap-2">
            <button type="button" onClick={() => setLocModal(false)} className="px-4 py-2 rounded-xl border border-slate-200 text-sm text-slate-600 hover:bg-slate-50">Cancel</button>
      ...
      <Modal isOpen={roomModal} onClose={() => setRoomModal(false)} title={editRoomId ? 'Edit Room' : `Add Room — ${selectedLocation?.name ?? ''}`}>
        <form onSubmit={handleRoomSubmit} className="p-1 space-y-4">
          <div>
            <label className="block text-xs font-medium text-slate-600 mb-1">Room / Office Name <span className="text-red-400">*</span></label>
          ...
          {saveError && <p className="text-sm text-red-500 bg-red-50 rounded-xl px-3 py-2">{saveError}</p>}
          <div className="flex justify-end gap-2">
            <button type="button" onClick={() => setRoomModal(false)} className="px-4 py-2 rounded-xl border border-slate-200 text-sm text-slate-600 hover:bg-slate-50">Cancel</button>
      ...
      <Modal isOpen={!!deleteLocId} onClose={() => setDeleteLocId(null)} title="Delete Building">
        <div className="p-1 space-y-4">
          <p className="text-sm text-slate-600">This will delete the building and all its rooms. Assets assigned here will lose their location. Are you sure?</p>
          <div className="flex justify-end gap-2">
            <button onClick={() => setDeleteLocId(null)} className="px-4 py-2 rounded-xl border border-slate-200 text-sm text-slate-600 hover:bg-slate-50">Cancel</button>
      ...
      <Modal isOpen={!!deleteRoomId} onClose={() => setDeleteRoomId(null)} title="Delete Room">
        <div className="p-1 space-y-4">
          <p className="text-sm text-slate-600">Are you sure you want to delete this room? Assets assigned here will lose their room assignment.</p>
          <div className="flex justify-end gap-2">
            <button onClick={() => setDeleteRoomId(null)} className="px-4 py-2 rounded-xl border border-slate-200 text-sm text-slate-600 hover:bg-slate-50">Cancel</button>

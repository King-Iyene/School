...
const COLORS = [
  ...
  { value: 'slate',   label: 'Grey',    bg: 'bg-slate-100',   text: 'text-slate-700'   },
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm">Group assets by type — furniture, bedding, electronics and more</p>
        </div>
      ...
      {!loading && categories.length === 0 && (
        <div className="bg-white rounded-2xl border border-slate-200 p-6">
          <p className="text-sm font-medium text-slate-600 mb-3">Suggested categories to get started:</p>
          <div className="flex flex-wrap gap-2">
            {SUGGESTED.map(s => (
              <button key={s} onClick={() => { setForm({ name: s, color: 'slate' }); setEditId(null); setSaveError(''); setModalOpen(true); }}
                className="text-sm px-3 py-1.5 rounded-full border border-dashed border-slate-300 text-slate-500 hover:border-emerald-400 hover:text-emerald-600 transition-colors">
                + {s}
              </button>
            ))}
          </div>
        ...
        {loading ? (
          ...
          <div className="divide-y divide-slate-100">
            {categories.map(c => {
              ...
              return (
                <div key={c.id} className="flex items-center gap-4 px-5 py-3.5 hover:bg-slate-50 transition-colors group">
      ...
      <Modal isOpen={!!deleteId} onClose={() => setDeleteId(null)} title="Delete Category">
        <div className="p-1 space-y-4">
          <p className="text-sm text-slate-600">Assets in this category will become uncategorised. Delete anyway?</p>
          <div className="flex justify-end gap-2">

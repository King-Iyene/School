      ...
      <div className="bg-app-surface rounded-2xl border border-app-border overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-app-text-muted">No room types found.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead className="bg-app-surface-alt border-b border-app-border">
                <tr>
                  ...
                  <th className="text-left px-4 py-3 font-semibold text-app-text-muted">Cost Per Term</th>
                  <th className="text-left px-4 py-3 font-semibold text-app-text-muted">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-app-border">
                {roomTypes.map(rt => (
                  <tr key={rt.id} className="hover:bg-app-surface-alt/50">
                    <td className="px-4 py-3 font-medium text-app-text">{rt.name}</td>
                    <td className="px-4 py-3 text-app-text-muted">{rt.description || '-'}</td>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editId ? 'Edit Room Type' : 'Add Room Type'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-app-text mb-1">Capacity (beds)</label>
              <input
                type="number"
                min="1"
                required

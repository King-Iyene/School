    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-bold text-app-text">Dormitory Rooms</h1>
        </div>
        <button
          onClick={openAdd}
      ...
      <div className="bg-app-surface rounded-2xl border border-app-border overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-app-text-muted">No rooms found.</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-app-surface-alt border-b border-app-border">
                <tr>
                  <th className="text-left px-4 py-3 font-semibold text-app-text-muted">Room No</th>
                  ...
                  <th className="text-left px-4 py-3 font-semibold text-app-text-muted">Cost/Term</th>
                  <th className="text-left px-4 py-3 font-semibold text-app-text-muted">Floor</th>
              ...
              <tbody className="divide-y divide-app-border">
                {rooms.map(room => (
                  <tr key={room.id} className="hover:bg-app-surface-alt/50">
                    <td className="px-4 py-3 font-medium text-app-text">{room.room_no}</td>
                    <td className="px-4 py-3 text-app-text-muted capitalize">{room.room_type}</td>
                    <td className="px-4 py-3 text-app-text-muted">{room.capacity ?? '-'}</td>
                    <td className="px-4 py-3 text-app-text font-medium">
                      {room.cost_per_term != null ? `₦${Number(room.cost_per_term).toLocaleString()}` : '-'}
                    </td>
                    <td className="px-4 py-3 text-app-text-muted">{room.floor || '-'}</td>
                    <td className="px-4 py-3 text-app-text-muted">{room.building || '-'}</td>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editId ? 'Edit Room' : 'Add Room'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-app-text mb-1">Room No</label>
              <input
                required
            ...
            <div>
              <label className="block text-sm font-medium text-app-text mb-1">Room Type</label>
              <select
                className={INPUT_CLASS}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-app-text mb-1">Cost per Term (₦)</label>
              <input
                type="number"
                min="0"
                step="0.01"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-app-text mb-1">Floor</label>
              <input
                className={INPUT_CLASS}
                value={form.floor}
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="room_is_active" className="text-sm text-app-text">Active</label>
          </div>

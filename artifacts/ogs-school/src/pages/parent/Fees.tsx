...
export default function ParentFees() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-xl font-bold text-app-text">Fee Status</h2>
        <p className="text-app-text-muted text-sm">View payment history and outstanding balances</p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-app-surface rounded-2xl border border-app-border shadow-sm p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm text-app-text-muted">Pending</p>
              <p className="text-xl font-bold text-app-text">{pendingCount}</p>
            </div>
          </div>
        </div>
        <div className="bg-app-surface rounded-2xl border border-app-border shadow-sm p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm text-app-text-muted">Transactions</p>
      ...
      <div className="bg-app-surface rounded-2xl border border-app-border shadow-sm overflow-hidden">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-app-border">
            {loading ? (
              <tr><td colSpan={6} className="text-center py-8 text-app-text-muted">Loading...</td></tr>
            ) : payments.length === 0 ? (
              <tr><td colSpan={6} className="text-center py-8 text-app-text-muted">No payment records found</td></tr>
            ) : payments.map(p => (
              <tr key={p.id} className="hover:bg-app-surface-alt transition-colors">
                ...
                <td className="px-5 py-3 text-sm text-app-text-muted font-mono">{p.receipt_number || '—'}</td>
              </tr>
            ))}
          </tbody>
        </table>

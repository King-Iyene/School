            ...
            <tbody className="divide-y divide-slate-100">
              {accounts.map((acc) => (
                <tr key={acc.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-slate-600">{acc.bank_name}</td>
                  <td className="px-4 py-3 font-mono text-slate-600">{maskAccountNumber(acc.account_number)}</td>
                  <td className="px-4 py-3 text-slate-500">{acc.branch}</td>
                  <td className="px-4 py-3 text-slate-700 font-medium">{formatCurrency(acc.opening_balance)}</td>

...
interface StudentRow {
  ...
  lastName: string;
  studentId: string;
  scores: Record<string, { ca1: number; ca2: number; ca3: number; exam: number; total: number; grade: string; excluded: boolean; hasRow: boolean }>;
  aggregate: number;
  average: number;
  position: number;
}
...
export default function ClassResults() {
  ...
  async function loadResults() {
    ...
    setLoading(true);
    try {
      const [subjectRes, enrollRes, gradesRes, compilationRes, exclusionRes] = await Promise.all([
      ...
      // Term-aware exclusion set: term_id null = whole year, else must match the selected term.
      const exclusionSet = new Set<string>(
        (exclusionRes.data ?? [])
          .filter((e: any) => e.term_id == null || e.term_id === selectedTerm)
      ...
      const rows: StudentRow[] = students.map(s => {
        ...
        enrichedSubjects.forEach(sub => {
          ...
          const excluded = exclusionSet.has(`${s.id}:${sub.id}`);
          const hasRow = !!g;
          scores[sub.id] = { ca1, ca2, ca3, exam, total, grade: gradeFromTotal(total), excluded, hasRow };
          // A compiled grade row counts even when total is 0 (missed exam). Only skip
          // subjects with no grade row at all, or excluded subjects.
          if (hasRow && !excluded) { aggregate += total; subjectCount++; }
        });
...
<tr>
  ...
  ${subjects.map(s => {
    ...
    // Excluded subjects render like normal rows (no marker); they are already
    // skipped from aggregate/average/position above.
    if (!sc || !sc.hasRow) return `<td>-</td>`;
    return `<td>${sc.total}</td>`;
  }).join('')}
                ...
                {studentRows.map((row, idx) => (
                  <tr key={row.id} className={`hover:bg-slate-50/60 transition-colors ${idx % 2 === 0 ? '' : 'bg-slate-50/20'}`}>
                    ...
                    {subjects.map(s => {
                      ...
                      const hasRow = sc && sc.hasRow;
                      // Excluded subjects render like normal rows (no marker); they are
                      // already skipped from aggregate/average/position above.
                      return (

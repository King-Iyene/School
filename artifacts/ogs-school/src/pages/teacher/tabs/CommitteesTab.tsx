    ...
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-100 flex items-center justify-between">
          <h4 className="font-bold text-slate-800 flex items-center gap-2 text-sm"><Shield className="w-4 h-4 text-blue-600" />Committee Memberships</h4>
        ...
        {committees.length === 0 ? (
          <div className="text-center py-8 text-slate-400 text-sm">No committee memberships recorded</div>
        ) : (
          <div className="divide-y divide-slate-100">
            {committees.map(c => (
              <div key={c.id} className="px-5 py-3.5 flex items-start justify-between gap-3 hover:bg-slate-50">
                <div className="flex items-start gap-3">
                  <div className={`w-2.5 h-2.5 rounded-full mt-1.5 flex-shrink-0 ${c.is_active ? 'bg-emerald-500' : 'bg-slate-300'}`} />
                  <div>
                    <p className="text-sm font-bold text-slate-800">{c.committee_name}</p>
                    <p className="text-xs text-slate-500 capitalize">{c.position}{c.start_date ? ` · from ${new Date(c.start_date).getFullYear()}` : ''}{c.end_date ? ` to ${new Date(c.end_date).getFullYear()}` : ''}{c.is_active ? ' · Active' : ' · Inactive'}</p>
                    {c.notes && <p className="text-xs text-slate-400 mt-0.5">{c.notes}</p>}
                  </div>
                </div>
                {isAdmin && (
                  <div className="flex gap-1">
                    <button onClick={() => openEditCom(c)} className="p-1.5 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg"><Edit2 className="w-3.5 h-3.5" /></button>
      ...
      {isAdmin && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="p-4 border-b border-slate-100 flex items-center justify-between">
            <h4 className="font-bold text-slate-800 flex items-center gap-2 text-sm"><AlertTriangle className="w-4 h-4 text-red-500" />Disciplinary Records <span className="text-xs font-normal text-slate-400">(Confidential)</span></h4>
          ...
          {disciplinary.length === 0 ? (
            <div className="text-center py-8 text-slate-400 text-sm flex flex-col items-center gap-2"><CheckCircle className="w-8 h-8 text-emerald-200" />No disciplinary records</div>
          ) : (
            <div className="divide-y divide-slate-100">
              {disciplinary.map(d => (
                <div key={d.id} className="px-5 py-3.5 flex items-start justify-between gap-3 hover:bg-slate-50">
                  <div>
                    <div className="flex items-center gap-2 mb-0.5">
                      <p className="text-sm font-bold text-slate-800">{d.incident_type}</p>
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium capitalize ${statusColor[d.status] ?? 'bg-slate-100 text-slate-600'}`}>{d.status}</span>
                    </div>
                    <p className="text-xs text-slate-500">{new Date(d.incident_date).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}</p>
                    {d.description && <p className="text-xs text-slate-600 mt-1">{d.description}</p>}
                    {d.action_taken && <p className="text-xs text-slate-500 mt-0.5">Action: {d.action_taken}</p>}
                    {d.sanction && <p className="text-xs text-red-600 mt-0.5 font-medium">Sanction: {d.sanction}</p>}
                  </div>
                  <div className="flex gap-1">
                    <button onClick={() => openEditDisc(d)} className="p-1.5 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg"><Edit2 className="w-3.5 h-3.5" /></button>
      ...
      <Modal isOpen={showComModal} onClose={() => setShowComModal(false)} title={editCom ? 'Edit Committee' : 'Add Committee Membership'} size="md">
        <div className="space-y-3">
          <div>
            ...
            <select value={comForm.committee_name} onChange={e => setComForm({ ...comForm, committee_name: e.target.value })} className={`${inputCls} bg-white`}>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div><label className={labelCls}>Position</label>
              <select value={comForm.position} onChange={e => setComForm({ ...comForm, position: e.target.value })} className={`${inputCls} bg-white`}>
            ...
            <div><label className={labelCls}>Status</label>
              <select value={comForm.is_active ? 'active' : 'inactive'} onChange={e => setComForm({ ...comForm, is_active: e.target.value === 'active' })} className={`${inputCls} bg-white`}>
          ...
          <div className="flex gap-2 pt-1">
            <button onClick={() => setShowComModal(false)} className="flex-1 border border-slate-200 text-slate-700 rounded-xl py-2 text-sm hover:bg-slate-50">Cancel</button>
      ...
      <Modal isOpen={showDiscModal} onClose={() => setShowDiscModal(false)} title={editDisc ? 'Edit Disciplinary Record' : 'Add Disciplinary Record'} size="md">
        <div className="space-y-3">
          ...
          <div><label className={labelCls}>Status</label>
            <select value={discForm.status} onChange={e => setDiscForm({ ...discForm, status: e.target.value })} className={`${inputCls} bg-white`}>
          ...
          <div className="flex gap-2 pt-1">
            <button onClick={() => setShowDiscModal(false)} className="flex-1 border border-slate-200 text-slate-700 rounded-xl py-2 text-sm hover:bg-slate-50">Cancel</button>

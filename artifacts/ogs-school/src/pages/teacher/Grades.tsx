...
export default function TeacherGrades() {
  ...
  async function loadGrades() {
    setLoading(true);
    const [enrollRes, gradeRes, exclRes, pubRes] = await Promise.all([
    ...
    // Students marked as not offering this subject are hidden from the mark sheet
    // (exclusion with term_id null applies to every term).
    const excluded = new Set(
      (exclRes.data ?? [])
        .filter(e => e.term_id == null || e.term_id === selectedTerm)
        .map(e => e.student_id),
    );
    setLocked(!!pubRes.data && !canEditPublished);
    setStudents((enrollRes.data ?? []).map(e => e.students).filter(Boolean).filter((s: any) => !excluded.has(s.id)));
    ...
    <div className="space-y-5">
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          {locked && (
            <div className="mx-4 my-3 flex items-center gap-2 bg-amber-50 border border-amber-200 rounded-xl px-4 py-2.5 text-sm text-amber-700">
              <XCircle className="w-4 h-4 flex-shrink-0" />
              <span>Results for this class and term have been published — scores are locked. Only the Super Admin or Principal can make changes.</span>
            </div>
          )}
          {saveError && (

...
export default function ParentGrades() {
  ...
  async function loadGrades() {
    setLoading(true);
    const { data } = await supabase.from('grades').select('*, subjects(name, code)').eq('student_id', selectedChild).eq('term_id', selectedTerm).order('total_score', { ascending: false });
    setGrades(data ?? []);
    setLoading(false);
  }
          ...
          <tbody className="divide-y divide-slate-100">
            {loading ? (
              ...
              <tr key={g.id} className="hover:bg-slate-50 transition-colors">
                <td className="px-5 py-3 text-sm font-medium text-slate-800">{(g.subjects as any)?.name}</td>

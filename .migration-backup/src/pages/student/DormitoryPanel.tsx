...
export default function DormitoryPanel() {
  ...
  if (!assignment) {
    return (
      <div className="space-y-6">
        <h1 className="text-2xl font-bold text-slate-800">Dormitory</h1>
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-12 text-center">
          <div className="w-16 h-16 bg-slate-100 rounded-2xl flex items-center justify-center mx-auto mb-4">
            <Building2 className="w-8 h-8 text-slate-400" />
          </div>
          <h3 className="font-semibold text-slate-700 text-lg">Not Assigned to Dormitory</h3>
          <p className="text-sm text-slate-400 mt-2 max-w-sm mx-auto">You have not been assigned to any dormitory room. Contact the school administration for dormitory arrangements.</p>
  ...
  const statusColor = assignment.status === 'active' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600';
        ...
        <div className="p-6">
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
            <div className="bg-slate-50 rounded-xl p-3.5">
              ...
              <p className="font-semibold text-slate-800">{room?.room_number || assignment.room_number || '—'}</p>
            </div>
            <div className="bg-slate-50 rounded-xl p-3.5">
              <p className="text-xs text-slate-500 mb-1">Room Type</p>
              <p className="font-semibold text-slate-800 capitalize">{room?.room_type || '—'}</p>
            </div>
            <div className="bg-slate-50 rounded-xl p-3.5">
              <p className="text-xs text-slate-500 mb-1">Bed Number</p>
              <p className="font-semibold text-slate-800">{assignment.bed_number || '—'}</p>
            </div>
            <div className="bg-slate-50 rounded-xl p-3.5">
              <p className="text-xs text-slate-500 mb-1">Floor</p>
              <p className="font-semibold text-slate-800">{room?.floor != null ? `Floor ${room.floor}` : '—'}</p>
            </div>
            <div className="bg-slate-50 rounded-xl p-3.5">
              <p className="text-xs text-slate-500 mb-1">Building Gender</p>
              <p className="font-semibold text-slate-800 capitalize">{building?.gender || '—'}</p>
            </div>
            <div className="bg-slate-50 rounded-xl p-3.5">
              <p className="text-xs text-slate-500 mb-1">Room Capacity</p>
              <p className="font-semibold text-slate-800">{room?.capacity ? `${room.capacity} beds` : '—'}</p>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {roommates.map((rm, i) => {
              ...
              return (
                <div key={rm.id} className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-slate-800 truncate">{rp?.first_name} {rp?.last_name}</p>
                    {rp?.admission_number && <p className="text-xs text-slate-500">ID: {rp.admission_number}</p>}
                  </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
          ...
          <div className="space-y-2.5">
            <div className="flex items-center justify-between py-2 border-b border-slate-100">
              <span className="text-sm text-slate-500">Building Name</span>
              <span className="text-sm font-semibold text-slate-800">{building.name || '—'}</span>
            </div>
            <div className="flex items-center justify-between py-2 border-b border-slate-100">
              <span className="text-sm text-slate-500">Gender</span>
              <span className="text-sm font-semibold text-slate-800 capitalize">{building.gender || '—'}</span>
            </div>
            <div className="flex items-center justify-between py-2">
              <span className="text-sm text-slate-500">Contact</span>
              <span className="text-sm font-semibold text-slate-800">{building.contact_number || 'Contact School'}</span>
            </div>
          </div>
        </div>
      )}

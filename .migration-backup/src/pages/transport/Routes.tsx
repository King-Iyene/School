      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-slate-500">No transport routes found.</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  <th className="text-left px-4 py-3 font-semibold text-slate-600">Route Name</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {routes.map(route => (
                  <tr key={route.id} className="hover:bg-slate-50/50">
                    <td className="px-4 py-3 font-medium text-slate-800">{route.route_name}</td>
                    <td className="px-4 py-3 text-slate-500 max-w-xs truncate">{route.description || '-'}</td>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editId ? 'Edit Route' : 'Add Route'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="is_active_route" className="text-sm text-slate-700">Active</label>
          </div>

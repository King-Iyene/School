...
export default function Classes() {
  ...
  const inputCls = 'w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/30 bg-white';
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm">Manage school classes and assign class teachers</p>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 flex flex-wrap gap-3">
        <input
          ...
          className="flex-1 min-w-[160px] border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/30"
        />
        <select value={filterLevel} onChange={e => setFilterLevel(e.target.value)} className="border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none bg-white">
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-slate-100 bg-slate-50">
              ...
              <th className="text-left text-xs font-semibold text-slate-500 uppercase px-5 py-3">Class Name</th>
              <th className="text-left text-xs font-semibold text-slate-500 uppercase px-5 py-3">Level</th>
              <th className="text-left text-xs font-semibold text-slate-500 uppercase px-5 py-3">Section</th>
              <th className="text-left text-xs font-semibold text-slate-500 uppercase px-5 py-3">Form Master</th>
              <th className="text-left text-xs font-semibold text-slate-500 uppercase px-5 py-3">Students</th>
              <th className="text-left text-xs font-semibold text-slate-500 uppercase px-5 py-3">Capacity</th>
              <th className="text-left text-xs font-semibold text-slate-500 uppercase px-5 py-3">Actions</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
            {loading ? (
              <tr><td colSpan={8} className="text-center py-10 text-slate-400">Loading classes...</td></tr>
            ) : filtered.length === 0 ? (
              <tr><td colSpan={8} className="text-center py-10 text-slate-400">{classes.length === 0 ? 'No classes created yet' : 'No classes match the filter'}</td></tr>
            ) : filtered.map((c, idx) => {
              ...
              return (
                <tr key={c.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-5 py-3.5">
                    <span className="text-sm font-semibold text-slate-800">{c.name || `${c.level}${c.section}`}</span>
                  </td>
                  <td className="px-5 py-3.5">
                    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${levelColors[c.level] ?? 'bg-slate-100 text-slate-600'}`}>
                      {c.level}
                    </span>
                  </td>
                  <td className="px-5 py-3.5 text-sm text-slate-600">{c.section || '—'}</td>
                  <td className="px-5 py-3.5 text-sm text-slate-600">
                    {teacher ? `${teacher.first_name} ${teacher.last_name}` : <span className="text-slate-400 italic">Unassigned</span>}
                  </td>
                  <td className="px-5 py-3.5">
                  ...
                  <td className="px-5 py-3.5 text-sm text-slate-600">{c.capacity}</td>
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-1">
                      <button onClick={() => openEdit(c)} title="Edit" className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors">
                        <Edit2 className="w-4 h-4" />
                      </button>
                      <button onClick={() => deleteClass(c.id)} title="Delete" className="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors">
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={editClass ? 'Edit Class' : 'Add New Class'}>
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Class Name <span className="text-slate-400 font-normal">(optional — defaults to level + section)</span></label>
            <input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder={`${form.level}${form.section}`} className={inputCls} />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Form Master</label>

...
import { ArrowLeft, User, BookOpen, UserCheck, DollarSign, AlertTriangle, Building2, History, ShoppingBag, GraduationCap, Phone, Mail, MapPin, Calendar, Users, Award, Printer, CreditCard as Edit2, Save, Trash2, FileText, ChevronRight } from 'lucide-react';
...
function overallRemark(avg: number): string {
...
export default function StudentProfile() {
  ...
  useEffect(() => {
    if (!enrollment?.class_id || !student) return;
    loadTab(tab);
  ...
  async function loadTab(t: string, overrideClassId?: string) {
    ...
    if (!classId) { setTabLoaded(prev => ({ ...prev, [t]: true })); return; }
      ...
      for (const tr of termsList) {
        ...
        if (tr.class_id) {
          const { data: classGrades } = await supabase
            .from('grades')
            .select('student_id, total_score, ca1_score, ca3_score, exam_score')
    ...
    if (t === 'subjects') {
      const { data } = await supabase.from('class_subjects').select('*, subjects(id, name, code, category), profiles(first_name, last_name)').eq('class_id', classId);
      setSubjects(data ?? []);
    }
    ...
    if (t === 'history' && canView.history) {
      const { data } = await supabase.from('student_enrollments').select('*, classes(name, level, section), academic_years(name), terms(name)').eq('student_id', studentId).order('enrollment_date', { ascending: false });
      setEnrollHistory(data ?? []);
    }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="pb-6">
        ...
        {tab === 'subjects' && (
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
            <h4 className="font-semibold text-slate-800 mb-4">Enrolled Subjects — {classLabel}</h4>
            {subjects.length === 0 ? (
                  ...
                  return (
                    <div key={s.id} className="border border-slate-100 rounded-xl p-4">
                      <div className="flex items-start justify-between gap-2 mb-2">
                        ...
                        {sub?.code && <span className="text-xs bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full">{sub.code}</span>}
                      </div>

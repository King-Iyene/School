...
export default function MarksRegister() {
  ...
  async function fetchStudents() {
    ...
    const [recordsRes, exclusionRes] = await Promise.all([
      supabase
    ...
    (exclusionRes.data ?? []).forEach((e: any) => { exMap[e.student_id] = e.id; });
  ...
  async function toggleOffers(idx: number) {
    ...
    if (student.offers) {
      // Mark as not offering
      if (student.record_id) {
        await supabase.from('exam_marks_records').delete().eq('id', student.record_id);
      }
      ...
      const { data } = await supabase.from('student_subject_exclusions').insert({
        school_id: schoolId,
      ...
      updateStudent(idx, 'record_id', undefined);
      updateStudent(idx, 'ca', '');
      updateStudent(idx, 'test', '');
      updateStudent(idx, 'exam', '');
    } else {
      if (student.exclusion_id) {
        await supabase.from('student_subject_exclusions').delete().eq('id', student.exclusion_id);
      }
  ...
  async function handleSaveAll() {
    ...
    const upserts = students.filter(s => s.offers).map((s) => {
    ...
    const { error } = await supabase
      .from('exam_marks_records')
      .upsert(upserts, { onConflict: 'exam_name_id,student_id,subject_id' });

    if (!error && selectedExamData?.term_id && selectedExamData?.academic_year_id) {
      const gradesUpserts = students.filter(s => s.offers).map((s) => {
        const caVal = parseFloat(s.ca) || 0;
        const testVal = parseFloat(s.test) || 0;
        const examVal = parseFloat(s.exam) || 0;
  ...
  async function handleGenerateResultCards() {
    ...
    const [schoolRes, enrollRes, marksRes, exclusionRes] = await Promise.all([
      ...
      selectedExamData?.academic_year_id
        ? supabase.from('student_subject_exclusions').select('student_id, subject_id').eq('academic_year_id', selectedExamData.academic_year_id)
        : Promise.resolve({ data: [] as any[] }),
    ]);
    const exclusionSet = new Set<string>((exclusionRes.data ?? []).map((e: any) => `${e.student_id}:${e.subject_id}`));
    ...
    enrollments.forEach(e => {
      const sm = allMarks.filter(m => m.student_id === e.student_id && !m.is_absent && !isExcluded(e.student_id, m.subject_id));
      const scoringSubjects = sm.filter(m => ((m.ca1 || 0) + (m.ca3 || 0) + (m.exam || 0)) > 0);
    ...
    const cardsHTML = enrollments.map(e => {
      ...
      const studentMarks = allMarks.filter(m => m.student_id === e.student_id && !isExcluded(e.student_id, m.subject_id));
      const totalScore = studentTotals[e.student_id] || 0;
      const subjectCount = studentMarks.filter(m => !m.is_absent && ((m.ca1||0)+(m.ca3||0)+(m.exam||0)) > 0).length;
      ...
      const subjectRows = subjectIds.map(sid => {
        if (isExcluded(e.student_id, sid)) return '';
        ...
        // Skip subjects with zero total — no meaningful result
        if (total === 0) return '';
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .school-sub { font-size: 11px; margin-top: 3px; color: #333; }
    .motto { font-size: 11px; font-style: italic; margin-top: 2px; }
    .report-title { font-size: 14px; font-weight: bold; margin-top: 8px; background: #1a1a1a; color: #fff; padding: 4px 10px; display: inline-block; letter-spacing: 0.5px; }
            ...
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  ...
                  <th className="text-center px-6 py-4 font-bold text-slate-700">Status</th>
                </tr>
              </thead>
                    ...
                    <tr key={student.student_id} className={`hover:bg-slate-50 transition-colors ${!student.offers ? 'bg-slate-100/60 opacity-60' : student.is_absent ? 'bg-red-50/30' : ''}`}>
                      ...
                      <td className="px-3 py-4 text-center">
                        <input
                          ...
                          value={student.ca}
                          disabled={student.is_absent || !student.offers}

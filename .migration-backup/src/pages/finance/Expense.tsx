...
export default function Expense() {
  ...
  function openDelete(rec: ExpenseRecord) {
    setDeleteTarget(rec);
    setDeleteModalOpen(true);
  }

  async function handleSave() {
    if (!form.source_name.trim() || !form.account_id || !form.expense_date || !form.amount) {
      setError('Source name, account, date, and amount are required.');
      return;
    }
    setSaving(true);
    setError('');
  ...
  async function handleDelete() {
    ...
    setSaving(true);
    await supabase.from('expense_records').delete().eq('id', deleteTarget.id);
    setSaving(false);
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editId ? 'Edit Expense' : 'Add Expense'}>

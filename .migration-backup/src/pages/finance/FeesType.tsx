  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-slate-800">Fees Type</h1>
        <button
          onClick={openCreate}
            ...
            <tbody className="divide-y divide-slate-100">
              {types.map((t) => (
                <tr key={t.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-4 py-3 font-mono text-xs text-slate-600 bg-slate-50 rounded">{t.fees_code}</td>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editId ? 'Edit Fees Type' : 'Add Fees Type'}>
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1">Fees Code</label>
              <input
                className={INPUT_CLASS}

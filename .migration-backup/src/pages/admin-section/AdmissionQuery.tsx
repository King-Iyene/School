  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-slate-800">Admission Queries</h2>
          <p className="text-slate-500 text-sm">Track and manage prospective student inquiries</p>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 flex flex-wrap gap-3">
        ...
        <select value={filterSource} onChange={e => setFilterSource(e.target.value)} className="border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none bg-white">
          <option value="">All Sources</option>
          {sources.map(s => <option key={s.id} value={s.name}>{s.name}</option>)}
        </select>
        <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none bg-white">
          <option value="">All Status</option>
          {STATUS_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
        </select>
        <input type="date" value={fromDate} onChange={e => setFromDate(e.target.value)} className="border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none" />
        <input type="date" value={toDate} onChange={e => setToDate(e.target.value)} className="border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none" />
      </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-slate-100 bg-slate-50">
              ...
              <th className="text-left text-xs font-semibold text-slate-500 uppercase px-5 py-3">Query Date</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {loading ? (
              ...
              <tr><td colSpan={8} className="text-center py-10 text-slate-400">No queries found</td></tr>
            ) : filtered.map((q, idx) => (
              <tr
                key={q.id}
              ...
              >
                ...
                <td className="px-5 py-3.5">
                  ...
                  {q.description && (
                    <p className="text-xs text-slate-400 mt-0.5 truncate max-w-[160px]">{q.description}</p>
                  )}
                </td>
                <td className="px-5 py-3.5 text-sm text-slate-600">{q.phone || '—'}</td>
                <td className="px-5 py-3.5 text-sm text-slate-600">{q.source || '—'}</td>
                <td className="px-5 py-3.5">
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={editItem ? 'Edit Admission Query' : 'Add Admission Query'} size="lg">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Class Interested</label>
      ...
      <Modal isOpen={showFollowUpModal} onClose={() => setShowFollowUpModal(false)} title="Add Follow-up" size="md">
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Note <span className="text-red-500">*</span></label>
            <textarea
          ...
          <div className="flex gap-3 pt-2">
            <button onClick={() => setShowFollowUpModal(false)} className="flex-1 px-4 py-2.5 border border-slate-200 text-slate-700 rounded-xl text-sm font-medium hover:bg-slate-50 transition-colors">
              Cancel
            </button>
            <button

...
export default function HRTab({ profileId, schoolId, profile, onProfileUpdate }: Props) {
  ...
  async function loadHR() {
    ...
    if (data) {
      setHr(data);
      setHrForm({
        nok_name: data.nok_name ?? '', nok_relationship: data.nok_relationship ?? '',
        nok_phone: data.nok_phone ?? '', nok_email: data.nok_email ?? '',
        nok_address: data.nok_address ?? '',
        emergency_contact_name: data.emergency_contact_name ?? '',
        emergency_contact_phone: data.emergency_contact_phone ?? '',
        emergency_contact_relationship: data.emergency_contact_relationship ?? '',
        bank_name: data.bank_name ?? '', account_number: data.account_number ?? '',
        account_name: data.account_name ?? '', bvn: data.bvn ?? '',
        pfa_name: data.pfa_name ?? '', pfa_number: data.pfa_number ?? '',
  ...
  async function saveProfile() {
    ...
    const { data, error } = await supabase.rpc('update_profile', {
      p_id: profileId,
      p_payload: { ...pForm, updated_at: new Date().toISOString() }
    }).select().maybeSingle();
    setSaving('');
  ...
  async function savePayroll() {
    ...
    const { data, error } = await supabase.rpc('update_profile', {
      p_id: profileId,
      p_payload: { ...payrollForm, updated_at: new Date().toISOString() }
    }).select().maybeSingle();
    setSaving('');
          ...
          <div className="space-y-4">
            <div>
              ...
              <dl className="grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-2">
                {[
                  { l: 'Name', v: hr?.nok_name || '—' },
                  { l: 'Relationship', v: hr?.nok_relationship || '—' },
                  { l: 'Phone', v: hr?.nok_phone || '—' },
                  { l: 'Email', v: hr?.nok_email || '—' },
                  { l: 'Address', v: hr?.nok_address || '—' },
          ...
          {editPayroll ? (
            ...
            <div className="space-y-4">
              <div>
                ...
                <dl className="grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-2">
                  {[
                    { l: 'Bank', v: hr?.bank_name || '—' },
                    { l: 'Account No.', v: hr?.account_number || '—' },
                    { l: 'Account Name', v: hr?.account_name || '—' },
                    { l: 'PFA', v: hr?.pfa_name || '—' },

    ...
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-100 flex items-center justify-between">
          <h4 className="font-bold text-slate-800 flex items-center gap-2 text-sm"><GraduationCap className="w-4 h-4 text-emerald-600" />Academic & Professional Qualifications</h4>
        ...
        {quals.length === 0 ? (
          <div className="text-center py-8 text-slate-400 text-sm">No qualifications recorded yet</div>
        ) : (
          <div className="divide-y divide-slate-100">
            {quals.map(q => (
              <div key={q.id} className="px-5 py-3.5 flex items-start justify-between gap-3 hover:bg-slate-50">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500">{QUAL_TYPES.find(t => t.value === q.qualification_type)?.label} {q.field_of_study ? `— ${q.field_of_study}` : ''}</p>
                    {q.institution && <p className="text-xs text-slate-500 mt-0.5">{q.institution}{q.grade_class ? ` · ${q.grade_class}` : ''}{q.year_obtained ? ` · ${q.year_obtained}` : ''}</p>}
                  </div>
                </div>
                {isAdmin && (
                  <div className="flex items-center gap-1">
                    <button onClick={() => openEditQual(q)} className="p-1.5 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg"><FileText className="w-3.5 h-3.5" /></button>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-100">
          <h4 className="font-bold text-slate-800 flex items-center gap-2 text-sm mb-3"><FileText className="w-4 h-4 text-emerald-600" />Documents & Certificates</h4>
          {isAdmin && (
            <div className="bg-slate-50 rounded-xl p-3 flex flex-wrap items-end gap-2">
              <div className="flex-1 min-w-32">
                <label className="block text-xs font-semibold text-slate-500 mb-1">Document Type</label>
                <select value={uploadDocType} onChange={e => setUploadDocType(e.target.value)} className={`${inputCls} bg-white`}>
        ...
        {docs.length === 0 ? (
          ...
          <div className="divide-y divide-slate-100">
            {docs.map(d => (
              <div key={d.id} className="px-5 py-3.5 flex items-center justify-between gap-3 hover:bg-slate-50">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-xl bg-amber-50 flex items-center justify-center flex-shrink-0">
                    <FileText className="w-4 h-4 text-amber-600" />
                  </div>
                  <div>
                    <p className="text-sm font-bold text-slate-800">{d.file_name}</p>
                    <p className="text-xs text-slate-500">{docTypeLabel(d.document_type)}{d.description ? ` · ${d.description}` : ''} · {new Date(d.uploaded_at).toLocaleDateString('en-GB')}</p>
                  </div>
                </div>
                <div className="flex items-center gap-1">
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select value={qualForm.qualification_type} onChange={e => setQualForm({ ...qualForm, qualification_type: e.target.value })} className={`${inputCls} bg-white`}>
                {QUAL_TYPES.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-500 uppercase mb-1">Year Obtained</label>
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-500 uppercase mb-1">Title / Award *</label>
          ...
          <div className="flex gap-2 pt-2">
            <button onClick={() => setShowQualModal(false)} className="flex-1 border border-slate-200 text-slate-700 rounded-xl py-2 text-sm hover:bg-slate-50">Cancel</button>

...
function getOverallRemark(avg: number): string {
...
export default function StudentReportPrint({ studentId, termId, academicYearId, onClose }: Props) {
  ...
  async function loadData() {
    ...
    const exclusionsQuery = resolvedYearId
      ? supabase.from('student_subject_exclusions').select('student_id, subject_id').eq('academic_year_id', resolvedYearId)
      : Promise.resolve({ data: [] as any[] });

    const [gradesRes, attRes, classGradesRes, exclusionsRes] = await Promise.all([
      gradesQuery,
    ...
    const exclusionSet = new Set<string>(((exclusionsRes as any).data ?? []).map((e: any) => `${e.student_id}:${e.subject_id}`));

    const allGrades = gradesRes.data ?? [];
    const filteredGrades = allGrades.filter(g => {
      if (exclusionSet.has(`${g.student_id}:${g.subject_id}`)) return false;
      const total = g.total_score ?? ((g.ca1_score || 0) + (g.ca3_score || 0) + (g.exam_score || 0));
      return total > 0;
    });
    setGrades(filteredGrades);

    const allClassGrades: any[] = (classGradesRes as any).data ?? [];
    ...
    if (resolvedTermId && resolvedYearId) {
      ...
      if (cmtData?.outstanding_fees_override != null) {
        ...
        const totalDue = (feesRes.data ?? []).reduce((s, r: any) => s + Number(r.amount || 0), 0);
        const totalPaid = (paidRes.data ?? []).reduce((s, r: any) => s + Number(r.net_amount || 0), 0);
        setOutstandingFees(Math.max(0, totalDue - totalPaid));
      }
    }
  ...
  const overallTotal = grades.reduce((sum, g) => {
    const t = g.total_score ?? ((g.ca1_score || 0) + (g.ca3_score || 0) + (g.exam_score || 0));
    return sum + (t || 0);
  }, 0);
  const overallAvg = grades.length > 0 ? overallTotal / grades.length : 0;
  const overallRemark = grades.length > 0 ? getOverallRemark(overallAvg) : '';
  const remarkColor =
      ...
      <div id="print-report">
        <div className="print-page">
          <div className="flex justify-end mb-3 print:hidden">
            <button
              onClick={onClose}
              className="px-4 py-2 text-sm bg-slate-100 hover:bg-slate-200 rounded-lg text-slate-700 mr-2 transition-colors"
            >
              Close
            </button>
            <button
              ...
              <div style={{
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '4px' }}>
                  <div style={{ textAlign: 'center', padding: '2px', borderRight: '1px solid #e2e8f0' }}>
                    ...
                    <div style={{ fontSize: '6.5pt', color: '#94a3b8' }}>/{grades.length * 100}</div>
                  </div>
                  <div style={{ textAlign: 'center', padding: '2px', borderRight: '1px solid #e2e8f0' }}>
                    ...
                    <div style={{ fontSize: '6.5pt', color: '#94a3b8' }}>{grades.length} subj</div>
                  </div>
                  <div style={{ textAlign: 'center', padding: '2px', borderRight: '1px solid #e2e8f0' }}>
                    ...
                    {grades.map(g => {
                      ...
                      return (
                        <tr key={g.id}>
                          ...
                          <td style={{ textAlign: 'center', fontWeight: 'bold', color: '#1a3a5c' }}>
                            {subPos ? `${getOrdinal(subPos.pos)}/${subPos.size}` : '—'}
                          </td>
                          <td style={{ textAlign: 'center', fontWeight: 'bold' }}>{g.grade ?? '—'}</td>
                          <td style={{ textAlign: 'center' }}>{gradeText(g.grade)}</td>
                        </tr>
                      );
                    })}
                  </tbody>
              ...
              <div style={{ borderTop: '1px dashed #eee', paddingTop: '3px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: '8px', alignItems: 'flex-end' }}>
                  ...
                  <span style={{
                    flex: 1,
                    fontSize: '8.5pt',
                    fontStyle: comments?.principal_comment ? 'normal' : 'italic',
                    color: comments?.principal_comment ? '#1a1a1a' : '#aaa',
                  }}>
                    {comments?.principal_comment || '___________________________________'}
                  </span>
